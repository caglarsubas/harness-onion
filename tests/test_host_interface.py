...
@pytest.mark.parametrize('key', ['reviewGaps', 'modules', 'backlog', 'obligations', 'budgets',
    ...
    'implementationReady', 'w01Complete', 'requestActionCorrelationProven', 'interfaces', 'requiredOpenDesignGates'])
...
def test_current_catalog_checked_before_history(authority, fault):
    ...
    if fault == 'missing': packets.pop('MET-ENFORCE-002')
    if fault == 'extra': packets['UNKNOWN-001'] = {}
    if fault == 'changed': packets['MET-ENFORCE-002']['allowedPaths'].append('host-enforcement/')
    if fault == 'historical': packets = module.historical_catalog(packets)
    if fault == 'owner': packets['MET-ENFORCE-002']['repository'] = 'mas-harness-operator'
...
def test_inputs_are_exact_current_data(authority, fault):
    ...
    if fault == 'packet': packets['MET-ENFORCE-002']['offlineAcceptanceCommands'].pop()
...
def test_fresh_history_refuses_stale_actual_bytes(authority, monkeypatch):
    ...
    assert module._record()['authorityPacket'] == 'MET-ENFORCE-002'
...
def test_prior_contracts_and_evidence_are_preserved(authority):
    ...
    assert len(authority[0]['MET-ENFORCE-002']['offlineAcceptanceCommands']) == 49
