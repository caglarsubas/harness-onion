...
Status: **CORRECTED_AWAITING_HASH_BOUND_REREVIEW**. This file is authored by the
implementation/design agent; it is not an independent verdict. The first exact
draft received CHANGES_REQUIRED for request-to-action correlation; see the retained
review record. The prior OBS-ARCH-DESIGN-001 review does not extend to these new
interface and topology proposals.
...
| G09 | REQUEST_ACTION_CORRELATION: a buffered duplicate of completed action A can match a later identical action B in the same execution/generation | Unchanged I04 is not proven sufficient. Demonstrate an immutable-profile/transport restriction excluding ambiguity, or explicitly review a versioned protocol amendment before adoption; no new field now and no caller actionId as authority |
...
17. Action A completes while a duplicate remains buffered in TLS/parser/kernel
    state; byte-identical action B is armed in the same execution/generation; the
    delayed duplicate then consumes B. Required: prove request-to-action binding
    under this interleaving, or leave G09 open and the interface unadopted. One
    outstanding action, a run certificate and request digest are not that proof.
...
| T02 lifecycle | unarmed/duplicate/cross-case action, old socket, queued action, conflicting gate/server result, same-generation delayed-identical-request across two armed actions | Source tests of actual owner implementations |
...
PASS_FOR_SOURCE_PUBLICATION must still identify G04-G07 and G09 as unresolved if they are
