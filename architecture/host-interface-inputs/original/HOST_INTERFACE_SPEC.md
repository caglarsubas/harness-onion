...
4. The gate authenticates the original run-specific server certificate, parses the
   ...
   own signed profile, and matches exactly the sole armed action. A caller cannot
   choose an actionId through an HTTP header; no new public wire field is needed
   for this candidate. Duplicate or ambiguous matches deny rather than guess.
