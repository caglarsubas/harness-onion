...
[Guard-traversal successor](../docs/alpha-2/GUARD_TRAVERSAL_REPAIR.md): Alpha2 ONGOING.
...
[Bounded factory diagnostics](../docs/alpha-2/FACTORY_DIAGNOSTICS.md): Alpha2 ONGOING.
...
[Completion integration](../docs/alpha-2/COMPLETION_INTEGRATION.md): Alpha2 ONGOING.
...
[One additional local acceptance attempt](../docs/alpha-2/LOCAL_ACCEPTANCE_REVALIDATION.md) is current.
...
| 99 | `MET-PERF-002` | `Harness-Engineering` | Measured conformance performance repair authority |
| 100 | `CONF-PERF-001` | `mas-harness-conformance-labs` | Bounded private arithmetic optimization and exact source accounting |
| 101 | `MET-PERF-003` | `Harness-Engineering` | Exact historical-consumer and profiling authority follow-up |
| 102 | `CONF-PERF-002` | `mas-harness-conformance-labs` | Eight-path measured repair; supersedes unaccepted draft13 dispatch |
| 103 | `MET-PERF-004` | `Harness-Engineering` | Four fixed accounting bridges and transitive source-consumer inventory |
| 104 | `CONF-PERF-003` | `mas-harness-conformance-labs` | Complete original-crypto baseline before any private arithmetic repair |
| 105 | `MET-PERF-005` | `Harness-Engineering` | Completed reference benchmark and unchanged full candidate gates |
| 106 | `CONF-PERF-004` | `mas-harness-conformance-labs` | Single source owner: scaffold then bounded measured candidate |
| 107 | `CONF-BENCH-001` | `mas-harness-conformance-labs` | Read-only committed-scaffold reference replay; no branch/PR or merge authority |
| 108 | `MET-REPAIR-016` | `Harness-Engineering` | Exact successor checkpoint compatibility authority |
| 109 | `CONF-FIX-006` | `mas-harness-conformance-labs` | Two-path inventory test and independent correction binding |
| 110 | `CONF-LIVE-003` | `mas-harness-conformance-labs` | Fixed proxy transport and zero-cost admission |
| 111 | `CONF-LIVE-004` | `mas-harness-conformance-labs` | Native Linux build and ten fixed probes |
| 112 | `CONF-LIVE-005` | `mas-harness-conformance-labs` | Reproducible packaging and operator handoff |
| 113 | `CONF-LIVE-006` | `mas-harness-conformance-labs` | Trusted campaign integration and manual qualification declaration |
| 114 | `CTRL-INTEGRATE-001` | `mas-harness-control-plane` | Alpha-1 production overview integration carryover |
| 115 | `MODEL-001` | `mas-harness-model-plane` | Local inference core |
| 116 | `MODEL-002` | `mas-harness-model-plane` | Custody and signed routes |
| 117 | `MODEL-OLLAMA-001` | `mas-harness-model-plane` | Ollama provider |
| 118 | `MODEL-LLAMACPP-001` | `mas-harness-model-plane` | llama.cpp provider |
| 119 | `MODEL-VLLM-001` | `mas-harness-model-plane` | vLLM provider |
| 120 | `MODEL-003` | `mas-harness-model-plane` | Model security and telemetry |
| 121 | `KN-RET-001` | `mas-harness-knowledge-plane` | Retrieval and cited context |
| 122 | `EXEC-001` | `mas-harness-execution-plane` | Execution foundation |
| 123 | `EXEC-PROT-001` | `mas-harness-execution-plane` | Protocol gateway |
| 124 | `EXEC-ORCH-001` | `mas-harness-execution-plane` | Durable orchestration |
| 125 | `RUN-001` | `mas-harness-runtime-plane` | Runtime edge foundation |
| 126 | `RUN-GW-001` | `mas-harness-runtime-plane` | Signed routing and budgets |
...
| Order | Packet | Repository | Acceptance slice |
|---:|---|---|---|
| 129 | `TRUST-GOV-001` | `mas-harness-trust-plane` | Approvals, autonomy, and waivers |
| 130 | `TRUST-REG-001` | `mas-harness-trust-plane` | AgentOps registry and promotion |
| 131 | `EXEC-TOOL-001` | `mas-harness-execution-plane` | Governed tools and compensation |
| 132 | `EXEC-SBX-001` | `mas-harness-execution-plane` | Job and Wasmtime sandboxes |
| 133 | `EXEC-SBX-002` | `mas-harness-execution-plane` | gVisor and Kata sandboxes |
| 134 | `EXEC-ML-001` | `mas-harness-execution-plane` | Local decision service |
| 135 | `KN-MEM-001` | `mas-harness-knowledge-plane` | Governed memory |
| 136 | `RUN-EXP-001` | `mas-harness-runtime-plane` | Interaction and resumable UI |
| 137 | `OP-004` | `mas-harness-operator` | Per-module reconciliation |
| 138 | `CONF-A3-001` | `mas-harness-conformance-labs` | Governed-action certification |

## Alpha 4 — Enterprise release

| Order | Packet | Repository | Acceptance slice |
|---:|---|---|---|
| 139 | `MODEL-004` | `mas-harness-model-plane` | Performance evidence |
| 140 | `TRUST-EVAL-001` | `mas-harness-trust-plane` | Assurance and evaluation |
| 141 | `TRUST-003` | `mas-harness-trust-plane` | Trust resilience |
| 142 | `KN-002` | `mas-harness-knowledge-plane` | Knowledge resilience |
| 143 | `EXEC-002` | `mas-harness-execution-plane` | Execution resilience |
| 144 | `RUN-002` | `mas-harness-runtime-plane` | Runtime resilience |
| 145 | `DIST-005` | `mas-harness-distribution` | Distribution reproducibility/security |
| 146 | `OP-005` | `mas-harness-operator` | Upgrade and rollback |
| 147 | `OP-006` | `mas-harness-operator` | Uninstall and fleet sync |
| 148 | `OP-007` | `mas-harness-operator` | Platform security |
| 149 | `CONF-K8S-001` | `mas-harness-conformance-labs` | Kubernetes live certification |
| 150 | `CONF-OCP-001` | `mas-harness-conformance-labs` | OpenShift live certification |
| 151 | `CONF-K3S-001` | `mas-harness-conformance-labs` | K3s certification |
| 152 | `CONF-AIR-001` | `mas-harness-conformance-labs` | Physical air-gap certification |
...
| 176 | `MET-REPAIR-018` | `Harness-Engineering` | Separate completion integration authority |
| 177 | `CONF-FIX-008` | `mas-harness-conformance-labs` | Bounded completion successor after accepted helper repair |

| 178 | `MET-PERF-014` | `Harness-Engineering` | Bounded factory diagnostic authority |
...
| 183 | `MET-PERF-017` | `Harness-Engineering` | Guard-traversal successor publication |
