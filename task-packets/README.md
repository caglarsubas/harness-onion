...
Alpha-2 publication snapshots. The current catalog has 122 packets; the first repair publication retains its 114-packet snapshot. Alpha-1
...
| 82 | `MODEL-001` | `mas-harness-model-plane` | Local inference core |
| 83 | `MODEL-002` | `mas-harness-model-plane` | Custody and signed routes |
| 84 | `MODEL-OLLAMA-001` | `mas-harness-model-plane` | Ollama provider |
| 85 | `MODEL-LLAMACPP-001` | `mas-harness-model-plane` | llama.cpp provider |
| 86 | `MODEL-VLLM-001` | `mas-harness-model-plane` | vLLM provider |
| 87 | `MODEL-003` | `mas-harness-model-plane` | Model security and telemetry |
| 88 | `KN-RET-001` | `mas-harness-knowledge-plane` | Retrieval and cited context |
| 89 | `EXEC-001` | `mas-harness-execution-plane` | Execution foundation |
| 90 | `EXEC-PROT-001` | `mas-harness-execution-plane` | Protocol gateway |
| 91 | `EXEC-ORCH-001` | `mas-harness-execution-plane` | Durable orchestration |
| 92 | `RUN-001` | `mas-harness-runtime-plane` | Runtime edge foundation |
| 93 | `RUN-GW-001` | `mas-harness-runtime-plane` | Signed routing and budgets |
| 94 | `RUN-GW-002` | `mas-harness-runtime-plane` | Streaming and cancellation |
| 95 | `CONF-A2-001` | `mas-harness-conformance-labs` | Read-only agent certification |

## Alpha 3 — Governed action and interaction

| Order | Packet | Repository | Acceptance slice |
|---:|---|---|---|
| 96 | `TRUST-GOV-001` | `mas-harness-trust-plane` | Approvals, autonomy, and waivers |
| 97 | `TRUST-REG-001` | `mas-harness-trust-plane` | AgentOps registry and promotion |
| 98 | `EXEC-TOOL-001` | `mas-harness-execution-plane` | Governed tools and compensation |
| 99 | `EXEC-SBX-001` | `mas-harness-execution-plane` | Job and Wasmtime sandboxes |
| 100 | `EXEC-SBX-002` | `mas-harness-execution-plane` | gVisor and Kata sandboxes |
| 101 | `EXEC-ML-001` | `mas-harness-execution-plane` | Local decision service |
| 102 | `KN-MEM-001` | `mas-harness-knowledge-plane` | Governed memory |
| 103 | `RUN-EXP-001` | `mas-harness-runtime-plane` | Interaction and resumable UI |
| 104 | `OP-004` | `mas-harness-operator` | Per-module reconciliation |
| 105 | `CONF-A3-001` | `mas-harness-conformance-labs` | Governed-action certification |

## Alpha 4 — Enterprise release

| Order | Packet | Repository | Acceptance slice |
|---:|---|---|---|
| 106 | `MODEL-004` | `mas-harness-model-plane` | Performance evidence |
| 107 | `TRUST-EVAL-001` | `mas-harness-trust-plane` | Assurance and evaluation |
| 108 | `TRUST-003` | `mas-harness-trust-plane` | Trust resilience |
| 109 | `KN-002` | `mas-harness-knowledge-plane` | Knowledge resilience |
| 110 | `EXEC-002` | `mas-harness-execution-plane` | Execution resilience |
| 111 | `RUN-002` | `mas-harness-runtime-plane` | Runtime resilience |
| 112 | `DIST-005` | `mas-harness-distribution` | Distribution reproducibility/security |
| 113 | `OP-005` | `mas-harness-operator` | Upgrade and rollback |
| 114 | `OP-006` | `mas-harness-operator` | Uninstall and fleet sync |
| 115 | `OP-007` | `mas-harness-operator` | Platform security |
| 116 | `CONF-K8S-001` | `mas-harness-conformance-labs` | Kubernetes live certification |
| 117 | `CONF-OCP-001` | `mas-harness-conformance-labs` | OpenShift live certification |
| 118 | `CONF-K3S-001` | `mas-harness-conformance-labs` | K3s certification |
| 119 | `CONF-AIR-001` | `mas-harness-conformance-labs` | Physical air-gap certification |
| 120 | `CONF-SEC-001` | `mas-harness-conformance-labs` | Adversarial security certification |
| 121 | `CONF-UPG-001` | `mas-harness-conformance-labs` | Lifecycle certification |
| 122 | `CONF-WG-001` | `mas-harness-conformance-labs` | White-goods enterprise acceptance |
...
The MET-REPAIR-003 publication contained 120 packets; the current catalog contains 122. The original 118-packet Linux policy and prior
...
The current catalog has 122 packets; the earlier 121-packet publication and all
source/native gates remain unchanged. No existing product assertion may change.
