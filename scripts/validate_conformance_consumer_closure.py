...
def validate_authority(packets, record, inputs):
    try:
        ...
        require(len(old) == 148 and len(packets) == 158 and set(packets) == old | set(NEW_IDS) | {"MET-PERF-005", "CONF-PERF-004", "CONF-BENCH-001", "MET-REPAIR-016", "CONF-FIX-006", "MET-ADOPT-001", "MET-PERF-006", "CONF-DIAG-001"}, "exact153 catalog")
...
def main():
    ...
    for error in errors:
        print("ERROR: "+error)
    if not errors:
        print("Conformance performance authority valid: 158 packets; 148 immutable YAML; 127/327 checkpoint; product/native NOT_RUN.")
