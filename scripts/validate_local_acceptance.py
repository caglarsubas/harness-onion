...
def validate_authority(packets, record, inputs):
    try:
        ...
        require(len(old) == 161 and len(packets) == 163 and set(packets) == old | set(NEW_IDS) | {'MET-PUBLISH-001'}, '161 unchanged plus one META')
...
if __name__ == '__main__':
    ...
    if errors:
        print('\n'.join(errors))
        raise SystemExit(1)
    print('Local acceptance authority valid:163 specifications;161 unchanged packets; one additional local-only attempt.')
