...
def validate_authority(packets, record, inputs):
    try:
        ...
        require(len(old) == 161 and len(packets) == 181 and set(packets) == old | set(NEW_IDS) | {'MET-PUBLISH-001', 'MET-REPAIR-017', 'CONF-FIX-007', 'MET-ADOPT-002', 'MET-PERF-010', 'MET-PERF-009', 'CONF-DIAG-003', 'MET-PERF-011', 'MET-PERF-012', 'CONF-PERF-006', 'CONF-BENCH-002', 'MET-PERF-013', 'CONF-BENCH-003', 'MET-REPAIR-018', 'CONF-FIX-008', 'MET-PERF-014', 'CONF-DIAG-004', 'MET-PERF-015', 'CONF-FIX-009'}, '161 unchanged plus one META')
...
if __name__ == '__main__':
    ...
    if errors:
        print('\n'.join(errors))
        raise SystemExit(1)
    print('Local acceptance authority valid:181 specifications;161 unchanged packets; one additional local-only attempt.')
