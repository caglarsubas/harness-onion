...
def historical_catalog(packets):
    ...
    require(type(packets) is dict and validate_additions(packets) == [], 'exact repair packet')
...
def validate_authority(packets, record, inputs):
    try:
        ...
        for path, rule in record['metaRecipes'].items():
            ...
            require(apply_recipe(before, rule) == inputs[path], 'checked reversible history')
