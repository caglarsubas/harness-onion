...
def validate_authority(packets, record, inputs):
    try:
        ...
        require(len(old) == 166 and len(packets) == 173
                and set(packets) == old | set(NEW_IDS) | {"MET-PERF-009", "CONF-DIAG-003", "MET-PERF-011", "MET-PERF-012", "CONF-PERF-006", "CONF-BENCH-002"}, "166 immutable predecessors plus one repair")
...
if __name__ == "__main__":
    ...
    if errors:
        print("\n".join(errors))
        raise SystemExit(1)
    print("Validation-performance authority valid:173 specifications;166 unchanged packets; "
