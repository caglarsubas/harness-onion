...
def validate_authority(packets, record, inputs):
    try:
        ...
        require(len(old) == 153 and len(packets) == 163 and set(packets) == old | set(NEW_IDS) | {"MET-ADOPT-001", "MET-PERF-006", "CONF-DIAG-001", "MET-PERF-007", "MET-PERF-008", "CONF-DIAG-002", "MET-ACCEPT-001", "MET-PUBLISH-001"}, "exact156 catalog")
...
def main():
    ...
    for error in errors:
        print("ERROR: " + error)
    if not errors:
        print("Successor checkpoint authority valid:163 packets;153 immutable YAML; two product paths; source data only.")
