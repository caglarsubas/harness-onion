...
def validate_qualification_authority(packets, record, inputs):
    try:
        ...
        require(len(old) == 143 and len(packets) == 150 and set(packets) == old | {"MET-REPAIR-015", "MET-PERF-002", "CONF-PERF-001", "MET-PERF-003", "CONF-PERF-002", "MET-PERF-004", "CONF-PERF-003"}, "exact catalog")
...
def main():
    ...
    for error in errors:
        print("ERROR: "+error)
    if not errors:
        print("Native qualification authority valid: 150 packets; 127/327 checkpoint; DATA_CHECK_ONLY; product/native NOT_RUN.")
