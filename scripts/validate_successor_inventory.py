...
def validate_successor_inventory(packets, record, inputs):
    try:
        ...
        if set(inputs) != set(pins):
            errors.append("exact 181 authority inputs required")
...
def main():
    ...
    for error in errors:
        print("ERROR: " + error)
    if not errors:
        print("Successor inventory authority valid: 181 packets; 181 predecessor files unchanged; product correction NOT_RUN; native gate closed.")
