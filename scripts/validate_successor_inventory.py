...
def validate_successor_inventory(packets, record, inputs):
    try:
        ...
        if set(inputs) != set(pins):
            errors.append("exact 185 authority inputs required")
...
def main():
    ...
    for error in errors:
        print("ERROR: " + error)
    if not errors:
        print("Successor inventory authority valid: 185 packets; 185 predecessor files unchanged; product correction NOT_RUN; native gate closed.")
