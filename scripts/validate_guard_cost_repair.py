...
def historical_bytes(path, raw):
    raw = accounting_history(path, raw)
...
def current_test_bytes(before):
    ...
    if not matches:
        ...
        return accounting_current(before)
    require(len(matches) == 1, "unique predecessor")
    return accounting_current(apply_recipe(before, matches[0]))


def validate_additions(packets):
    try:
        packets = accounting_catalog(packets)
        record = _record()
...
SPEC_PATH = 'architecture/guard-cost-repair.json'
SPEC_SHA256 = 'ffcc6d352aebe1ee9d87cc46af2c42699645744e83e21321e837a61839e5694d'
MEMBER_DIGESTS = {'schemaVersion': '9b44bf0f7e962e4a18a1bf7352a7d23ccb929447c297664b2814ab97194a489c', 'packetId': '5956bd8a0bdb36bdd29e9ac09ad526238779c81802d9117dce6286e73faeb75f', 'metaBase': '4aea4a95babd99a62e53730418cc9b61a092e19d90986f7152b7c90ea6cb902d', 'status': '0da9365f401908507ca93b0b0a3016a6beaadd8af1310b652f540dc34b73a11f', 'subject': '443a34275784fb6ffdd554ecfde60aed28d7deb12af7698e2dc49d9be9d8ff57', 'diagnostic': '2d21ba6147004dec5301e237bb32142fd2bb3189cb47b5e95c00ae727a331a06', 'repairPolicy': 'faf7aa58ed3dcc4b0e658faa44bbbb6650d04a9f0702479085d782b3ace1d09c', 'budgets': '48259aee5628fd9698fc16f08d0cceb337dda93564f56bfe8109c8ba56e08f98', 'limits': '55c877ff2cfdeb3570006b31f6a0bc121923356c6912855cccf6b8e7fa5cce47', 'gates': '91856b614e26f75c60e63a56ee1cda43b41da16466fdfbc1b9196b4d7a5cfb69', 'preservedArtifacts': 'fa03e93de845dafcb4c9435dbf2331d3a84eac42f9341ab9ecd8ca69c3fc26f1', 'dispatchOverlaps': '0db54a091181f2ad320bee02f6d54d5a7078394b24291949d02d35f454039e86', 'productExecutionInMetaRun': 'fcbcf165908dd18a9e49f7ff27810176db8e9f63b4352213741664245224f8aa', 'productAcceptance': 'fcbcf165908dd18a9e49f7ff27810176db8e9f63b4352213741664245224f8aa', 'nativeAcceptance': 'fcbcf165908dd18a9e49f7ff27810176db8e9f63b4352213741664245224f8aa', 'tenantAcceptance': 'fcbcf165908dd18a9e49f7ff27810176db8e9f63b4352213741664245224f8aa', 'phaseComplete': 'fcbcf165908dd18a9e49f7ff27810176db8e9f63b4352213741664245224f8aa', 'testAccounting': '23f4fe487c12c59b271896f5e024c83ac1bb2f893517f2a8b51d9bffc1393d8a'}
SOURCE = 'architecture/factory-diagnostics-inputs/static-source.json'
AUDIT = 'architecture/guard-cost-repair-inputs/audit.json'


def historical_catalog(packets):
    record = _record(); pinned(record)
    require(validate_additions(packets) == [], 'exact guard repair packets')
    packets = accounting_catalog(packets)
    old = {Path(p).stem for p in record['protectedFiles'] if p.startswith('task-packets/') and p.endswith('.yaml')}
    require(len(old) == 179 and set(packets) == old | set(NEW_IDS), 'closed 181-packet catalog')
    return {name: packets[name] for name in old}


def validate_spec(value, *, bind=True):
    require(type(value) is dict and set(value) == set(MEMBER_DIGESTS), 'closed guard repair specification')
    if bind: require(digest(canonical(value)) == SPEC_SHA256, 'exact guard repair specification')
    for key, checksum in MEMBER_DIGESTS.items():
        require(digest(canonical(value[key])) == checksum, 'changed repair boundary: '+key)
...
def close_guard_dispatch(packets, errors):
    try:
        ...
        for row in value['dispatchOverlaps']:
            ...
            for name in (a,b):
                p = 'task-packets/'+name+'.yaml'; raw = accounting_history(p,regular_bytes(ROOT,p))
            ...
            require(path in packets[a]['allowedPaths'] and path in packets[b]['allowedPaths'], 'exact inherited source path')
            exact.append('unordered same-repository packets '+a+' and '+b+' overlap at '+repr(path)+' and '+repr(path))
        require(exact and len(exact) == len(set(exact)) and all(errors.count(e) == 1 for e in exact), 'exact repair overlaps')
        return [e for e in errors if e not in exact]
    except (ValueError,TypeError,KeyError,OSError,RecursionError):
        return errors+['missing or changed guard repair dispatch']
...
def validate_authority(packets,record,inputs):
    try:
        ...
        for p,checksum in pins.items():
            require(type(inputs[p]) is bytes and digest(accounting_history(p,inputs[p])) == checksum, 'source drift: '+p)
        packets = accounting_catalog(packets)
        for name in packets:
            require(canonical(packets[name]) == canonical(safe_load(accounting_history('task-packets/'+name+'.yaml',inputs['task-packets/'+name+'.yaml']))), 'packet byte parity')
        value = parse(inputs[SPEC_PATH]); validate_spec(value)
        meta,product,prior = packets['MET-PERF-015'],packets['CONF-FIX-009'],old['MET-PERF-014']
        require(meta['repository'] == 'Harness-Engineering' and meta['predecessors'] == ['MET-PERF-014']
                and meta['allowedPaths'] == record['ownedPaths'], 'exact META owner')
        commands = meta['offlineAcceptanceCommands']
        require(len(commands) == 44 and commands[:-3]+commands[-2:] == prior['offlineAcceptanceCommands']
                and commands[-3] == ['uv','run','--offline','--frozen','--no-sync','python','scripts/validate_guard_cost_repair.py'], 'all43 inherited commands')
        require(product['repository'] == 'mas-harness-conformance-labs'
                and product['predecessors'] == ['MET-PERF-015','CONF-FIX-008','CONF-DIAG-004']
                and product['allowedPaths'] == old['CONF-FIX-008']['allowedPaths']
                and product['offlineAcceptanceCommands'] == old['CONF-FIX-008']['offlineAcceptanceCommands'], 'exact five paths and all8 commands')
        ...
        source,audit = parse(inputs[SOURCE]),parse(inputs[AUDIT])
        require(source['trackedFiles'] == 135 and source['totalTests'] == 1617 and source['backendTests'] == 1447, 'complete inherited inventory')
        require(audit['status'] == 'PARTIAL_DIAGNOSTIC_ONLY' and audit['completedCases'] == 1062 and audit['notStartedCases'] == 384
                and audit['sampledStarted'] == 1 and audit['sampledCompleted'] == 0 and audit['retriesRemaining'] == 0
                and audit['evidenceClass'] == 'WORKLOAD_DIAGNOSTIC_ONLY' and not audit['productAcceptance'], 'partial remains partial')
        require(audit['lastProgress']['callEvents'] == value['diagnostic']['calls']
                and audit['lastProgress']['returnEvents'] == value['diagnostic']['returns'], 'measured lower bounds only')
        ...
        for path,rule in record['metaRecipes'].items():
            ...
            require(apply_recipe(before,rule) == accounting_history(path,inputs[path]), 'exact reversible metadata')
            if path.startswith('tests/'):
                require(test_ids(before) == test_ids(inputs[path]), 'inherited test identities')
        for path in record['navigationPaths']:
            require(all(s in inputs[path] for s in (b'GUARD_COST_REPAIR.md',b'MET-PERF-015',b'CONF-FIX-009',b'CLOSED_PARTIAL_DIAGNOSTIC',b'HARNESS_PAPER_REPOSITORY_MAP.md',b'NOT_DUE')), 'consistent navigation')
        return []
    except (ValueError,TypeError,KeyError,AttributeError,UnicodeError,SyntaxError,RecursionError) as exc:
        return ['invalid guard cost repair authority: '+str(exc)]
...
if __name__ == '__main__':
    ...
    print('Guard cost repair authority valid:181 specifications;179 immutable packets; product repair not executed or accepted.')
