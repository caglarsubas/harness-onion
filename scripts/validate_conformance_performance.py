...
RECORD_SHA256 = "ec6725b60efa1b6cebb93b25cdce58ea09aa27d3d04cf467c6905d61319d8756"
...
def validate_authority(packets, record, inputs):
    try:
        ...
        require(len(old) == 144 and len(packets) == 146 and set(packets) == old | set(NEW_IDS), "exact146 catalog")
        for name in packets:
...
def main():
    ...
    for error in errors:
        print("ERROR: "+error)
    if not errors:
        print("Conformance performance authority valid: 146 packets; 144 immutable YAML; 127/327 checkpoint; product/native NOT_RUN.")
