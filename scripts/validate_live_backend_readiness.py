...
def validate_live_backend_readiness(
    ...
    try:
        ...
        if set(packets) != old_ids | set(NEW_IDS) | set(ADDITIONS) | set(SUCCESSORS) | set(PROXY_ADDITIONS) or len(packets) != 167:
            errors.append("current roadmap requires exactly 167 named packets; historical authority remains 130")
...
def main() -> int:
    ...
    for error in errors:
        print("ERROR: " + error)
    if not errors:
        print("Live backend roadmap valid: 167 packets; historical 130-packet authority and 156 predecessor files unchanged; source-only, native gate closed.")
    return bool(errors)
