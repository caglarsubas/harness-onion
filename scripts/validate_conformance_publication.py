...
def validate_authority(packets, record, inputs):
    try:
        ...
        require(len(old) == 162 and len(packets) == 173 and set(packets) == old | set(NEW_IDS) | {'MET-REPAIR-017','CONF-FIX-007', 'MET-ADOPT-002', 'MET-PERF-010', 'MET-PERF-009', 'CONF-DIAG-003', 'MET-PERF-011', 'MET-PERF-012', 'CONF-PERF-006', 'CONF-BENCH-002'}, '162 immutable plus one META')
...
if __name__ == '__main__':
    ...
    if errors:
        print('\n'.join(errors))
        raise SystemExit(1)
    print('Conformance publication authority valid:173 specifications;162 unchanged packets; bounded later CI and LOCAL exact-main.')
