...
def validate_authority(packets, record, inputs):
    try:
        ...
        require(len(old) == 146 and len(packets) == 155 and set(packets) == old | set(NEW_IDS) | {"MET-PERF-004", "CONF-PERF-003", "MET-PERF-005", "CONF-PERF-004", "CONF-BENCH-001", "MET-REPAIR-016", "CONF-FIX-006"}, "exact150 catalog")
...
def main():
    ...
    for error in errors:
        print("ERROR: "+error)
    if not errors:
        print("Conformance performance authority valid: 155 packets; 146 immutable YAML; 127/327 checkpoint; product/native NOT_RUN.")
