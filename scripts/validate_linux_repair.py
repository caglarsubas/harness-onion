...
def validate_linux_repair(packets: Any, amendment: Any, historical_policy_bytes: bytes) -> list[str]:
    ...
    if len(packets) != 120:
        errors.append("Linux repair requires exactly 120 packets")
    for packet_id, expected_digest in PACKET_DIGESTS.items():
        try:
...
def main() -> int:
    ...
    for error in errors:
        print("ERROR: " + error)
    if not errors:
        print("Linux repair authority valid: 120 packets; R1-R4 product fixes and native Linux remain unproven.")
