...
def validate_linux_repair(packets: Any, amendment: Any, historical_policy_bytes: bytes) -> list[str]:
    ...
    if len(packets) != 170:
        errors.append("Current catalog requires exactly 170 packets; consumed amendment remains 120")
...
def main() -> int:
    ...
    for error in errors:
        print("ERROR: " + error)
    if not errors:
        print("Linux repair authority valid: 170 packets; 120-packet record preserved; native Linux remains unproven.")
