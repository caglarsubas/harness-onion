...
def validate_linux_test_ownership(packets: Any, record: Any, historical_bytes: bytes) -> list[str]:
    ...
    if len(packets) != 169:
        errors.append("Current catalog requires exactly 169 packets; Linux ownership record remains 121")
...
def main() -> int:
    ...
    for error in errors:
        print("ERROR: " + error)
    if not errors:
        print("Linux test ownership valid: 169 packets; historical one-assertion grant preserved; native/live acceptance unproven.")
